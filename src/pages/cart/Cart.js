  ...
  return (
    <div className="cart-layout">
      ...
      <div className="cart-page">
        {amount === 0 && (
          <h2 className="cart-page-h2">YOUR SHOPPING CART IS EMPTY!!</h2>
        )}
        <div className="cart-page-left">
          <div className="cart">
            {cart.map((product) => {
              return <CartProduct key={product.id} {...product}></CartProduct>;
            })}
          </div>
        </div>
        <div className="cart-page-right">{amount !== 0 && <Checkout />}</div>
      </div>
    </div>
  );
}
